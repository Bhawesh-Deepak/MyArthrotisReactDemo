...
import { apibaseUrl, deleteUserStatusApi, getUserStatusListApi } from "../../Helpers/ApiUrlHelper";
...
export default function UserStatus() {
  ...
  const Columns = [
...

    {
      ...
      render: (record) => (<><button type="button" onClick={() => DeleteRecord(record)}>Delete</button>
        <Link to={`/UpsertUserStatus?id=${record.id}`}>Edit</Link></>)
    },
  ];

  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteUserStatusApi + record.id).then(resp => {
  ...
  const GetUserStatusList = () => {
    axios.get(apibaseUrl + getUserStatusListApi).then((resp) => {
        ...
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to='/UpsertUserStatus' className="btn btn-success">Add </Link>
            </div>
          </div>
        </div>
