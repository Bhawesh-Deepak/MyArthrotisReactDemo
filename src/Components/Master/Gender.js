...
export default function Gender() {
...

    {
      ...
      render: (record) => (
        <>
          ...
          <Link to={`/UpsertGender?id=${record.id}`}>Edit</Link>
        </>
      ),
    },
  ];
  ...
  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteGenderApi + record.id).then((resp) => {
    ...
    >
      <div className="row" style={{ marginBottom: "30px" }}>
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to="/UpsertGender" className="btn btn-success">
                Add{" "}
              </Link>
            </div>
          </div>
        ...
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <Table columns={Columns} dataSource={genderList}></Table>
          </div>
        </div>
      </div>
    </div>
