...
import { apibaseUrl, GetUserTypeByIdApi, InsertUserTypeApi, UpdateUserTypeApi } from "../../Helpers/ApiUrlHelper";
import {Link} from 'react-router-dom';

export default function UpsertUserType() {
  const initialValue = {
    "id": 0,
    "name": "",
    "description": "",
  ...
  const GetUserTypeById=()=>{
    axios.get(apibaseUrl+ GetUserTypeByIdApi+ userTypeId).then(resp=>{
  ...
  const UpdateUserType=(values)=>{
    axios.post(apibaseUrl+ UpdateUserTypeApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }

  const InsertUserType=(values)=>{
    axios.post(apibaseUrl+ InsertUserTypeApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }
      ...
      <div className="row" style={{ marginBottom: "30px" }}>
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <h5 style={{ "text-align": "center" }}>Add UserType</h5>
          </div>
