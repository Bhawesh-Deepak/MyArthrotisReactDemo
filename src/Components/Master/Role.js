...
import { apibaseUrl, deleteRoleApi, getRoleListApi } from "../../Helpers/ApiUrlHelper";
...
export default function Role() {
  ...
  const Columns = [
...

    {
      ...
      render: (record) => (<><button type="button" onClick={() => DeleteRecord(record)}>Delete</button>
        <Link to={`/UpsertRole?id=${record.id}`}>Edit</Link></>)
    },
  ];

  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteRoleApi + record.id).then(resp => {
  ...
  const GetRoleList = () => {
    axios.get(apibaseUrl + getRoleListApi).then((resp) => {
        ...
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to='/UpsertRole' className="btn btn-success">Add </Link>
            </div>
          </div>
        </div>
