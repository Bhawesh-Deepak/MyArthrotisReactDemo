...
import { apibaseUrl, GetRoleByIdApi, InsertRoleApi, UpdateRoleApi } from "../../Helpers/ApiUrlHelper";
import {Link} from 'react-router-dom';

export default function UpsertRole() {
  const initialValue = {
    "id": 0,
    "name": "",
    "description": "",
  ...
  const GetRoleById=()=>{
    axios.get(apibaseUrl+ GetRoleByIdApi+ roleId).then(resp=>{
  ...
  const UpdateRole=(values)=>{
    axios.post(apibaseUrl+ UpdateRoleApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }

  const InsertRole=(values)=>{
    axios.post(apibaseUrl+ InsertRoleApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }
      ...
      <div className="row" style={{ marginBottom: "30px" }}>
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <h5 style={{ "text-align": "center" }}>Add Role</h5>
          </div>
