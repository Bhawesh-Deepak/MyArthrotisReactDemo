...
import { apibaseUrl, deleteSubscriptionApi, getSubscriptionListApi } from "../../Helpers/ApiUrlHelper";
...
export default function Subscription() {
  ...
  const Columns = [
...

    {
      ...
      render: (record) => (<><button type="button" onClick={() => DeleteRecord(record)}>Delete</button>
        <Link to={`/UpsertSubscription?id=${record.id}`}>Edit</Link></>)
    },
  ];

  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteSubscriptionApi + record.id).then(resp => {
  ...
  const GetSubscriptionList = () => {
    axios.get(apibaseUrl + getSubscriptionListApi).then((resp) => {
      setSubscriptionList(resp.data.response);
    });
  };
        ...
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to='/UpsertSubscription' className="btn btn-success">Add </Link>
            </div>
          </div>
        </div>
