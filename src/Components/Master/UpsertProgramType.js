...
import { apibaseUrl, GetProgramTypeByIdApi, InsertProgramTypeApi, UpdateProgramTypeApi } from "../../Helpers/ApiUrlHelper";
import {Link} from 'react-router-dom';

export default function UpsertProgramType() {
  const initialValue = {
    "id": 0,
    "name": "",
    "description": "",
  ...
  const GetProgramTypeById=()=>{
    axios.get(apibaseUrl+ GetProgramTypeByIdApi+ programTypeId).then(resp=>{
  ...
  const UpdateProgramType=(values)=>{
    axios.post(apibaseUrl+ UpdateProgramTypeApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }

  const InsertProgramType=(values)=>{
    axios.post(apibaseUrl+ InsertProgramTypeApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }
