...
import { apibaseUrl, GetUserStatusByIdApi, InsertUserStatusApi, UpdateUserStatusApi } from "../../Helpers/ApiUrlHelper";
import {Link} from 'react-router-dom';

export default function UpsertUserStatus() {
  const initialValue = {
    "id": 0,
    "name": "",
    "description": "",
  ...
  const GetUserStatusById=()=>{
    axios.get(apibaseUrl+ GetUserStatusByIdApi+ userStatusId).then(resp=>{
  ...
  const UpdateUserStatus=(values)=>{
    axios.post(apibaseUrl+ UpdateUserStatusApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }

  const InsertUserStatus=(values)=>{
    axios.post(apibaseUrl+ InsertUserStatusApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }
      ...
      <div className="row" style={{ marginBottom: "30px" }}>
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <h5 style={{ "text-align": "center" }}>Add UserStatus</h5>
          </div>
