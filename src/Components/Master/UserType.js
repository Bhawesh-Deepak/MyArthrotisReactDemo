...
import { apibaseUrl, deleteUserTypeApi, getUserTypeListApi } from "../../Helpers/ApiUrlHelper";
...
export default function UserType() {
  ...
  const Columns = [
...

    {
      ...
      render: (record) => (<><button type="button" onClick={() => DeleteRecord(record)}>Delete</button>
        <Link to={`/UpsertUserType?id=${record.id}`}>Edit</Link></>)
    },
  ];

  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteUserTypeApi + record.id).then(resp => {
  ...
  const GetUserTypeList = () => {
    axios.get(apibaseUrl + getUserTypeListApi).then((resp) => {
      setUserTypeList(resp.data.response);
    });
  };
        ...
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to='/UpsertUserType' className="btn btn-success">Add </Link>
            </div>
          </div>
        </div>
