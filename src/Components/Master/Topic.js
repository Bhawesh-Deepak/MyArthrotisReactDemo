...
import { apibaseUrl, deleteTopicApi, getTopicListApi } from "../../Helpers/ApiUrlHelper";
...
export default function Topic() {
  ...
  const Columns = [
...

    {
      ...
      render: (record) => (<><button type="button" onClick={() => DeleteRecord(record)}>Delete</button>
        <Link to={`/UpsertTopic?id=${record.id}`}>Edit</Link></>)
    },
  ];

  const DeleteRecord = (record) => {
    if (window.confirm(`Are you sure want to delete ${record.name} ?`)) {
      axios.get(apibaseUrl + deleteTopicApi + record.id).then(resp => {
  ...
  const GetTopicList = () => {
    axios.get(apibaseUrl + getTopicListApi).then((resp) => {
      setTopicList(resp.data.response);
    });
  };
        ...
        <div className="col-md-12">
          ...
          <div className="col-md-6">
            <div className="col-md-3">
              <Link to='/UpsertTopic' className="btn btn-success">Add </Link>
            </div>
          </div>
        </div>
