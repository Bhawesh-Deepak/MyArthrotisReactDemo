...
import { apibaseUrl, GetGenderByIdApi, InsertGenderApi, UpdateGenderApi } from "../../Helpers/ApiUrlHelper";
import {Link} from 'react-router-dom';

export default function UpsertGender() {
  const initialValue = {
    "id": 0,
    "name": "",
    "description": "",
  ...
  const GetGenderById=()=>{
    axios.get(apibaseUrl+ GetGenderByIdApi+ genderId).then(resp=>{
  ...
  const UpdateGender=(values)=>{
    axios.post(apibaseUrl+ UpdateGenderApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }

  const InsertGender=(values)=>{
    axios.post(apibaseUrl+ InsertGenderApi,values).then(resp=>{
      window.alert(resp.data.message);
    })
  }
      ...
      <div className="row" style={{ marginBottom: "30px" }}>
        <div className="row" style={{ marginBottom: "10px" }}>
          <div className="col-md-12">
            <h5 style={{ "text-align": "center" }}>Add Gender</h5>
          </div>
